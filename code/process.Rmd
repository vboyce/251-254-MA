...
gather_draws(`b_[a-zA-Z_(): ]+`, regex=TRUE) |> mutate(Term=str_sub(.variable, 3,-1) |> str_replace("M","_"), Estimate=.value) |> filter(Term!="Intercept") |> 
  ggplot(aes(y = reorder(Term, Estimate, mean), x = Estimate)) + geom_vline(xintercept=0)+
  stat_pointinterval(color="#440154")+labs(y="", x="Estimate, 66%, 95% CrI predicting replication in predictive interval")+theme(axis.text=element_text(size=11))
```
...
gather_draws(`b_[a-zA-Z_(): ]+`, regex=TRUE) |> mutate(Term=str_sub(.variable, 3,-1) |> str_replace("M","_"), Estimate=.value) |> filter(Term!="Intercept") |> 
  ggplot(aes(y = reorder(Term, Estimate, mean), x = Estimate)) + geom_vline(xintercept=0)+
  stat_pointinterval(color="#440154")+labs(y="", x="Estimate, 66%, 95% CrI predicting predictive interval with stats predictors")+theme(axis.text=element_text(size=11))
```
...
ggplot(to_plot, aes(x=time, y=value, color=var, label=var))+geom_line()+geom_label_repel(data=to_plot |> filter(time==70), min.segment.length = 100, nudge_x = 15, na.rm = TRUE)+
  annotate("text", x=40, y=.3,label="Predicting subjective replication", color="black", size=5, hjust=1)+scale_color_tableau("Classic 20")+
  coord_cartesian(xlim=c(0,90), ylim=c(-.2, .3))+theme(axis.text.x=element_blank(), axis.title.x=element_blank(), axis.ticks.x=element_blank(), legend.position="none")
...
ggplot(to_plot, aes(x=time, y=value, color=var, label=var))+geom_line()+geom_label_repel(data=to_plot |> filter(time==55), min.segment.length = 100, nudge_x = 15, na.rm = TRUE)+
      annotate("text", x=30, y=1.3,label="Predicting predictive interval", color="black", size=5, hjust=1)+ 
theme(legend.position="none")+ scale_color_tableau("Classic 20")+
...
ggplot(to_plot, aes(x=time, y=value, color=var, label=var))+geom_line()+geom_label_repel(data=to_plot |> filter(time==60), min.segment.length = 100, nudge_x = 15, na.rm = TRUE)+ theme(legend.position="none")+ scale_color_tableau("Classic 20")+
      annotate("text", x=40, y=.25,label="Predicting predictive interval", color="black", size=5, hjust=1)+
...
ggplot(dat, aes(x=sub_rep, fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(legend.position = "none", axis.text=element_text(size=11))+labs(y="Count", x="Subjective Replication score")

ggplot(dat, aes(x=subfield, fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme( axis.text=element_text(size=11), axis.title.x=element_blank())

dat |> mutate(open_mat=ifelse(open_mat, "Open materials", "No open materials"), open_data=ifelse(open_data, "Open data", "No open data")) |> ggplot( aes(x=open_data |> factor(), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+facet_wrap(.~open_mat)+theme(axis.title.x=element_blank())+theme( axis.text=element_text(size=11))
...
ggplot(dat, aes(x=ifelse(change_platform,"Switch to online","Same modality"), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme( axis.text=element_text(size=11), axis.title.x=element_blank())

ggplot(dat, aes(x=ifelse(is_within, "Within", "Between"), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(axis.title.x=element_blank())+theme( axis.text=element_text(size=11))

ggplot(dat, aes(x=ifelse(single_vignette, "1 vignette", "multiple vignettes"), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(axis.title.x=element_blank())+theme( axis.text=element_text(size=11))
```
