...
d_es_predInt |> 
  ...
  scale_color_viridis(discrete=F, direction = -1, 
                      name = "Subjective Replication Success")+
...
gather_draws(`b_[a-zA-Z_(): ]+`, regex=TRUE) |> mutate(Term=str_sub(.variable, 3,-1) |> str_replace("M","_"), Estimate=.value) |> 
  ...
  stat_pointinterval(color="#440154")+labs(y="", x="Estimate, 66%, 95% CrI")+theme(axis.text=element_text(size=11))
```
...
regularized
```{r}
predictors <- data_tier1 |> mutate(social=ifelse(subfield=="social", 1,0),other=ifelse(subfield=="other-psych",1,0), non=ifelse(subfield=="non-psych", 1,0)) |> 
  select(z_pub_year, open_data, open_mat, stanford, change_platform, social, other, non, 
...
#lasso_linear <- cv.glmnet(predictors, yvals |> as.matrix(),       family="gaussian", alpha=1)
...
to_plot <- lasso_linear2$beta |> as.matrix() |> as.tibble(rownames="var") |> pivot_longer(`s0`:`s71`) |> mutate(time=str_sub(name, 2,-1) |> as.numeric())


ggplot(to_plot, aes(x=time, y=value, color=var, label=var))+geom_line()+geom_label_repel(data=to_plot |> filter(time==70), min.segment.length = 100, nudge_x = 10, na.rm = TRUE)+ theme(legend.position="none")+ scale_color_tableau("Classic 20")+
  coord_cartesian(xlim=c(0,80), ylim=c(-.2, .3))+theme(axis.text.x=element_blank(), axis.title.x=element_blank(), axis.ticks.x=element_blank())
...
```{r}
dat <- data_tier2 |> filter(!is.na(p_orig)) |>  mutate(social=ifelse(subfield=="social", 1,0),other=ifelse(subfield=="other-psych",1,0), non=ifelse(subfield=="non-psych", 1,0))
predictors <- dat |> select(z_pub_year, open_data, open_mat, #stanford, 
                            change_platform, social, non, other,
                                   z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) |> as.matrix()
...
```{r}
to_plot <- lasso_linear_porig$beta |> as.matrix() |> as.tibble(rownames="var") |> pivot_longer(`s0`:`s72`) |> mutate(time=str_sub(name, 2,-1) |> as.numeric())
...
```{r}
to_plot <- lasso_linear_predint$beta |> as.matrix() |> as.tibble(rownames="var") |> pivot_longer(`s0`:`s58`) |> mutate(time=str_sub(name, 2,-1) |> as.numeric())


ggplot(to_plot, aes(x=time, y=value, color=var, label=var))+geom_line()+geom_label_repel(data=to_plot |> filter(time==55), min.segment.length = 100, nudge_x = 10, na.rm = TRUE)+ theme(legend.position="none")+ scale_color_tableau("Classic 20")+
  coord_cartesian(xlim=c(0,70))+theme(axis.text.x=element_blank(), axis.title.x=element_blank(), axis.ticks.x=element_blank())
...
sub_cor <- function(var, stat = "estimate") {
  if (stat == "estimate") {
    cor.test(data_tier1$sub_rep, pull(data_tier1, {{var}}))$estimate
  } else {
    cor.test(data_tier1$sub_rep, pull(data_tier1, {{var}}))$p.value
  }
}
...
ggplot(dat, aes(x=sub_rep, fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(legend.position = "none")+labs(y="Count", x="Subjective Replication score")

ggplot(dat, aes(x=subfield, fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)

dat |> mutate(open_mat=ifelse(open_mat, "Open materials", "No open materials"), open_data=ifelse(open_data, "Open data", "No open data")) |> ggplot( aes(x=open_data |> factor(), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+facet_wrap(.~open_mat)+theme(axis.title.x=element_blank())

 ggplot(dat, aes(x=factor(stanford), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)

ggplot(dat, aes(x=factor(change_platform), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)

ggplot(dat, aes(x=ifelse(is_within, "Within", "Between"), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(axis.title.x=element_blank())

ggplot(dat, aes(x=ifelse(single_vignette, "1 vignette", "multiple vignettes"), fill=sub_rep))+geom_bar(stat="count", position="dodge")+scale_fill_viridis(discrete=T)+theme(axis.title.x=element_blank())
```

```{r}
# using d for model to get non-z-scored version
ggplot(d_for_model, aes(x=pub_year, y=sub_rep, color=sub_rep))+geom_jitter()+geom_smooth(method="lm")+scale_color_viridis()

ggplot(d_for_model, aes(x=log_ratio_ss, y=log_sample, color=sub_rep))+geom_point()+scale_color_viridis()

ggplot(d_for_model, aes(x=log_ratio_ss, y=sub_rep, color=sub_rep))+geom_jitter()+geom_smooth(method="lm")+scale_color_viridis()

ggplot(d_for_model, aes(x=log_sample, y=sub_rep, color=sub_rep))+geom_jitter()+geom_smooth(method="lm")+scale_color_viridis()

ggplot(d_for_model, aes(x=log_trials, y=sub_rep, color=sub_rep))+geom_jitter()+geom_smooth(method="lm")+scale_color_viridis()
...
fake_data <- tribble(
  ...
  "cognitive", 1,         1,         0,                1,          0,                log(50),    log(100),
   "social", 0,         0,         1,                0,          1,                log(30),     log(1),
            ...
            z_log_ratio_ss=(log_ratio_ss-back_convert$mean_log_ratio_ss)/back_convert$sd_log_ratio_ss, sub_rep=NA) |> 
select(academic_year, sub_rep, 
         z_pub_year, subfield, open_data, open_mat, stanford, change_platform, z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) 
