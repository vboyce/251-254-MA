...
```{r}
d_val <- d |> filter(!status %in% c("missing", "non-experiment", "reproduction", "TODO"))
...
d_es_try <- d_es |> filter(!is.na(target_d_calc)&!is.na(rep_d_calc)|!is.na(target_beta)&!is.na(rep_beta)) |> 
  mutate(
         predInt=ifelse(!is.na(target_d_calc),
                        Replicate::pred_int(target_d_calc,target_se_calc**2, rep_d_calc, rep_se_calc**2)$rep.inside,
                        Replicate::pred_int(target_beta, target_SE**2, rep_beta, rep_SE**2)$rep.inside),
         p_orig=ifelse(!is.na(target_d_calc),
                       Replicate::p_orig(target_d_calc,target_se_calc**2, rep_d_calc, t2=0, rep_se_calc**2),
                       Replicate::p_orig(target_beta, target_SE**2,rep_beta, t2=0, rep_SE**2)))
...
d_check_nas <- d_es |> filter(!status %in% c("missing", "non-experiment", "reproduction")) |> ungroup() |>
...
message("number of complete rows")
