...
parse_t <- function(tval, within_between) {
  ...
  return(data.frame("df_1"=NA, "df_2"=df,"tstat"=val, "fstat"=NA, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=N_from_df))
}
```
...
parse_f <- function(fval, within_between){
  ...
  return(data.frame("df_1"=df_1,"df_2"=df_2,"tstat"=NA, "fstat"=val, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=N_from_df))
}
```
...
parse_chisq <- function(chival, within_between){
  ...
  return(data.frame("df_1"=df,"df_2"=NA,"tstat"=NA, "fstat"=NA, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=n))
}
...
parse_mean_ci <- function(mci,within_between,n){
  ...
  return(data.frame("df_1"=df,"df_2"=NA,"tstat"=tval, "fstat"=NA, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=NA))
}
```
...
parse_wilcoxon <- function(wilcox){
  ...
  d_calc=NA
    return(data.frame("df_1"=NA,"df_2"=NA,"tstat"=NA, "fstat"=NA, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=NA))
...
do_blanks <- function(){
  return(data.frame("df_1"=NA,"df_2"=NA,"tstat"=NA, "fstat"=NA, "p_calc"=NA, "d_calc"=NA, "N_calc"=NA))
}
...
```{r, message=T}
message("total rows")
d_es |> filter(!status %in% c("missing", "non-experiment", "reproduction")) |> nrow()

message("number of NAs per column")

d_es |> filter(!status %in% c("missing", "non-experiment", "reproduction")) |> ungroup() |> summarize(                                                      across(everything(),~sum(is.na(.)))) |>
  select(-rep_fstat, -rep_tstat,-rep_df_1,-rep_df_2,
         -target_fstat, -target_tstat, -target_df_1, -target_df_2,
         -d_fill_rep, -d_fill_target, 
         -replication_ES_type, -replication_ES_value, -replication_pvalue,
         -target_ES_type, -target_ES_value, -target_pvalue,
         -rep_N_calc, -target_N_calc,
         -replication_test, -target_test) |> 
