...
raw <- readxl::read_xlsx(here("data","raw_data.xlsx"), sheet="aligned data", skip=1) |> 
  mutate(student_name_shared=ifelse(is.na(student_name_shared), "N/A", student_name_shared),
         replication_report_included=ifelse(is.na(replication_report_included), "no", replication_report_included)) |> 
  select(target_lastauthor_year, student_name_shared, replication_report_included, how_close, sensitivity, include,
         status, status_explanation,  academic_year, journal, target_cite, article_link, study_number,  subfield, target_year, stanford_internal, 
         open_data, open_materials, target_on_turk, replication_on_turk, 
         within_between, single_vignette, repeated_measure,
         target_N, target_raw_stat, effect_description, same_dir, replication_N,
         replication_raw_stat, replicated_instructor_code, replicated_report_code,
         adjudicated_replication_code) |> write_csv(here("data","raw_data.csv"))
```

Read in cache.

```{r}
d <- read_csv(here("data", "raw_data.csv")) |> 
  select(target_lastauthor_year, sensitivity, include, how_close,
...
d |> filter(!status %in% c("missing", "non-experiment", "reproduction", "unusable")) |> tally()
...
```{r}
d_val <- d |> filter(!status %in% c("missing", "non-experiment", "reproduction", "unusable"))
```

# Closeness

```{r}
 d |> filter(include %in% c("exp","pred_int", "stats")) |> group_by(how_close) |> tally()
...
```{r}
#found error in pval calculation related to sign flips

test %>% ungroup() %>% summarize(target=quantile(target_p_calc, na.rm=T), rep=quantile(rep_p_calc, na.rm=T))
```
...
test |> filter(is.na(target_ES)&(is.na(target_d_calc))) |> filter(!is.na(target_raw_stat)) |> select(target_lastauthor_year, target_raw_stat)

test |> filter(is.na(rep_ES)&(is.na(rep_d_calc))) |> filter(!is.na(replication_raw_stat)) |> select(target_lastauthor_year, replication_raw_stat)
...
#d_es |> filter(is.na(target_ES)!=is.na(target_SE))
#d_es |> filter(is.na(rep_ES)!=is.na(rep_SE))
#d_es |> filter(is.na(rep_d_calc)!=is.na(rep_d_calc_se)) #fine all of the bad ones are exp only
#d_es |> filter(is.na(target_d_calc)!=is.na(target_d_calc_se))



d_es_predInt <- d_es |> 
  rowwise() |> 
  mutate(predInt = do_pred_int(target_ES, target_SE, rep_ES, rep_SE), # on untransformed scale
         predInt_from_d = do_pred_int(target_d_calc, target_d_calc_se, rep_d_calc, rep_d_calc_se), #on standardized scale 
         p_orig = do_p_orig(target_ES, target_SE, rep_ES, rep_SE),
         p_orig_from_d = do_p_orig(target_d_calc, target_d_calc_se, rep_d_calc, rep_d_calc_se),
         predInt=ifelse(is.na(predInt), predInt_from_d, predInt), #prefer untransformed scale, but use standardized when that's the only option
         p_orig=ifelse(is.na(p_orig), p_orig_from_d, p_orig), # prefer untrasformed when available 
         predInt_with_tau = do_pred_int_sens(target_d_calc, target_d_calc_se, rep_d_calc, rep_d_calc_se), #on standardized scale 
         p_orig_with_tau = do_p_orig_sens(target_d_calc, target_d_calc_se, rep_d_calc, rep_d_calc_se),
         sub_rep = do_subjective_rep(replicated_instructor_code, 
                                     replicated_report_code, 
                                     adjudicated_replication_code)) |> write_csv(here("data","processed_data.csv"))


#d_es_predInt |> filter(include!="exp") |> filter(is.na(predInt))
#d_es_predInt |> filter(include!="exp") |> filter(is.na(p_orig))
...
d_for_model <- d_es_predInt |> 
  ...
  mutate(pub_year= target_year-mean(target_year),
         log_p =ifelse(target_p_calc==0,-300, log(target_p_calc)), #prevent any -Inf problems!
  ...
  select(sensitivity, include, target_lastauthor_year, academic_year, subfield, 
         ...
         predInt, p_orig, sub_rep) |> write_csv(here("data","for_model.csv"))
...
```{r}
data_sens <- z_data |> mutate(sensitivity=ifelse(is.na(sensitivity), include, sensitivity))

data_sens1 <- data_sens |> filter(sensitivity %in% c("stats", "exp", "pred_int")) |>
  mutate(sub_rep=sub_rep*4+1) |>  #remap to 1-5 scale
  select(academic_year, sub_rep, 
         z_pub_year, subfield, open_data, open_mat, stanford, change_platform, z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) 

data_sens2 <- data_sens |> filter(sensitivity %in% c("stats", "pred_int")) |>
  select(academic_year, sub_rep, 
         p_orig, predInt,
         z_pub_year, subfield, open_data, open_mat, stanford, change_platform, z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) 

data_sens3 <- data_sens |>  filter(sensitivity %in% c("stats")) |>
  select(academic_year, sub_rep, 
         p_orig, predInt,
         z_target_d_calc, z_log_p, 
         z_pub_year, subfield, open_data, open_mat, stanford, change_platform, z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) 
...
```{r, eval=T}
sens_tier1_subjective <- brm(sub_rep ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                          z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                          (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                             z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials | academic_year),
                        family="cumulative", # runs on 1-5 data
                        data=data_sens1,
                        file=here(model_location, "sens_tier1_subjective"),
                        prior=priors, 
                        control=list(adapt_delta=.99),
                        backend="cmdstanr"
)
...
```{r, eval=T}
sens_tier2_predint <- brm(predInt ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                       z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                       (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                          z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials | academic_year),
                     family="bernoulli",
                     data=data_sens2,
                     file=here(model_location, "sens_tier2_predint"),
                     prior=priors, 
                     control=list(adapt_delta=.99),
                     backend="cmdstanr")
```
...
```{r, eval=T}
sens_tier2_porig <- brm(p_orig ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                     z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                     (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                        z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials | academic_year),
                   data=data_sens2,
                   file=here(model_location, "sens_tier2_porig"),
                   prior=priors, 
                   control=list(adapt_delta=.99),
                   backend="cmdstanr")
```
...
```{r, eval=T}
sens_tier3_subjective <- brm(sub_rep ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                          z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                          z_target_d_calc + z_log_p+ 
                          (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                             z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                             z_target_d_calc + z_log_p| academic_year),
                        family="cumulative",
                        data=data_sens3 |> mutate(sub_rep=sub_rep*4+1),
                        file=here(model_location, "sens_tier3_subjective"),
                        prior=priors, 
                        control=list(adapt_delta=.99),
                        backend="cmdstanr")
```
...
```{r, eval=T}
sens_tier3_predint <- brm(predInt ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                       z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                       z_target_d_calc + z_log_p+ 
                       (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                          z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                     ...
                     family="bernoulli",
                     data=data_sens3,
                     file=here(model_location, "sens_tier3_predint"),
                     prior=priors, 
                     control=list(adapt_delta=.99),
                     backend="cmdstanr")
```
...
```{r, eval=T}
sens_tier3_porig <- brm(p_orig ~ z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                     z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                     z_target_d_calc + z_log_p+ 
                     (z_pub_year + subfield + open_data +  open_mat + stanford + change_platform + 
                        z_log_ratio_ss + is_within + single_vignette + z_log_sample + z_log_trials + 
                        z_target_d_calc + z_log_p| academic_year),
                   data=data_sens3,
                   file=here(model_location, "sens_tier3_porig"),
                   prior=priors, 
                   control=list(adapt_delta=.99),
                   backend="cmdstanr"
)
...
<!-- 
## Frequentist Lassoes
Note these have not been updated since corrections to data pipeline 
```{r}
