...
#pool_m_sd(2.875,1.3102163,16,3.5,.8548504,14)
#pool_m_sd(1.5, 1.6329932, 16, 2.25, 1.6583124, 12)
#pool_m_sd(2.48,1.41,46,3.35,1.07,49)
#pool_m_sd(1.90,1.54,49,2.22,1.46,49)
```
# Parsing
...
parse_mean_ci <- function(mci,within_between,n){
  ...
  return(data.frame("df_1"=df,"df_2"=NA,"tstat"=tval, "fstat"=NA, "p_calc"=pval, "d_calc"=d_calc, "N_calc"=NA, "ES"=m1-m2, "SE"=se))
}
```
...
test |> filter(is.na(target_d_calc)&is.na(target_ES)) |> filter(!is.na(target_raw_stat)) |>  pull(target_raw_stat)

test |> filter(is.na(rep_d_calc)&is.na(rep_ES)) |> filter(!is.na(replication_raw_stat)) |> pull(replication_raw_stat)
```
...
TODOs:
- will need to do selection of which things have complete cases for models
- will need to z-score things (in a recoverable way)
