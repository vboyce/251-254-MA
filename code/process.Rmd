...
d <- readxl::read_xlsx(here("data","raw_data.xlsx"), 
  ...
  select(target_lastauthor_year, sensitivity, include,
         ...
         target_N, target_test, target_raw_stat, target_pvalue, target_ES_type, 
         target_ES_value, same_dir, replication_N, # replication_test
         replication_raw_stat, replication_pvalue, replication_ES_type, 
         replication_ES_value, replicated_instructor_code, replicated_report_code,
...
regularized
```{r}
predictors <- data_tier1 |> select(z_pub_year, open_data, open_mat, stanford, change_platform, 
                                   z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials)

yvals <- data_tier1 |> mutate(sub_rep=(sub_rep-1)/4) |> pull(sub_rep)
lasso_linear <- cv.glmnet(predictors, yvals |> as.matrix(), 
...
dat <- data_tier1 |> mutate(is.cog=ifelse(subfield=="cognitive", 1,0),
  ...
  select(is.cog, is.soc, is.other, is.non, z_pub_year, open_data, open_mat, stanford, change_platform, 
         z_log_ratio_ss, is_within, single_vignette, z_log_sample, z_log_trials) 
