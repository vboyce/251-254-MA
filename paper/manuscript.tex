...
Within-subjects designs stand out as the strongest indicator of replicability in the model with all the data (0.55, CrI= {[}-0.01, 2{]}). In the model with all predictors, but less data, within-subjects designs remain predictive (0.64, CrI= {[}-0.03, 2.38{]}), and standardized effect size is also a strong predictor of subjective replication score (0.59, CrI= {[}0.31, 2.58{]}). Both effects are robust to a sensitivity analysis including only studies with close replications and matching statistical tests (within-subjects 0.87, CrI= {[}-0.01, 3.34{]}; effect size 0.97, CrI= {[}0.76, 4.59{]}).
...
Open Science Consortium (2015) Estimating the reproducibility of psychological science. \emph{Science}
