...
```{r, include=F, echo=F}
d |> select(starts_with("pred") | starts_with("p_orig"))

d |> filter(!is.na(predInt)) |> count(predInt)
#using predInt as reported 62 in interval, 72 not (45% rep)

#of those with SMD predInts
d |> filter(!is.na(predInt_from_d)) |> count(predInt)
# using predInt 44 in / 70 out (38%)
d |> filter(!is.na(predInt_from_d)) |> count(predInt_from_d)
# 34 in /80 out (30%)
d |> filter(!is.na(predInt_from_d)) |> count(predInt_with_tau)
# with tau 73 in / 41 out (64%) 

d |> filter(!is.na(predInt)) |> summarise(median(p_orig))
#median .03

#of those with SMD predInts
d |> filter(!is.na(predInt_from_d)) |>summarise(median(p_orig))
# median .01
d |> filter(!is.na(predInt_from_d)) |> summarise(median(p_orig_from_d))
# median .001
d |> filter(!is.na(predInt_from_d)) |> summarise(median(p_orig_with_tau))
# median .15
```