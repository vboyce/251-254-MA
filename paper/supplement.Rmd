...
d |> filter(include %in% c("stats", "pred_int")) |> filter(!is.na(predInt)) |> count(predInt)

#of those with SMD predInts
d |>filter(include=="stats") |> filter(!is.na(predInt_from_d)) |> count(predInt)
d |> filter(include=="stats") |> filter(!is.na(predInt_from_d)) |> count(predInt_from_d)
d |> filter(include=="stats") |> filter(!is.na(predInt_from_d)) |> count(predInt_with_tau)

d |> filter(include %in% c("stats", "pred_int")) |> filter(!is.na(predInt)) |> summarise(median(p_orig))
#median .03

#of those with SMD predInts
d |> filter(include=="stats") |> filter(!is.na(predInt_from_d)) |>summarise(median(p_orig))
d |> filter(include=="stats") |> filter(!is.na(predInt_from_d)) |> summarise(median(p_orig_from_d))
d |> filter(include=="stats") |> filter(!is.na(predInt_from_d)) |> summarise(median(p_orig_with_tau))
```