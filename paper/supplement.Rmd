...
medians <- d %>% filter(include =="stats") %>% summarize(orig=quantile(target_d_calc), rep=quantile(rep_d_calc))

```


```{r forest, out.height="100%", fig.width=9, fig.height=12, fig.pos="ht", fig.cap="Forest plot of original and replication effect sizes (N=112). Original effect sizes are open dots, replication are closed dots. Coloring indicates subjective replication score, and p-original values are listed on the left side. The median original effect size was .61 and the median replication effect size was .28. "}
...
<!-- # Sub sample of Psych Science papers
...
We were unable to use journal as a predictor in the models because of very uneven representation across journals. By far the most represented journal was Psychological Science, with `r nrow(ps)` original papers. Other journals had at most a few papers in the sample. 
...
all_label <- all %>% select(target_lastauthor_year,journal)

desc <- d %>% left_join(all_label) %>% 
  mutate(journal=str_to_lower(journal)) |>
  filter(journal=="psych sci") %>% 
  mutate(target_d_calc=ifelse(include=="stats", target_d_calc, NA),
...
as_kable_extra(desc,format="latex", position="!h", caption="Descriptive properties of the Psych Science portion of the replication dataset. For categorical features, counts and percentages are given. For continuous features, the median value and interquartile range are given. ", booktabs=TRUE, linesep="") |> kable_styling(full_width=F, font_size=10)
...
library(kableExtra)
knitr::kable(cors, digits = 3, align='ccl',format="latex", position="!h", caption="The unadjusted Pearson correlations between each individual predictor and the subjective replication score for the Psych Science subsample.", booktabs=TRUE, linesep="") |> kable_styling(full_width=F, font_size=10) 
