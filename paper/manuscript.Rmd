...
sub_with_leg <- dat |> mutate(subfield=factor(subfield, levels=c("cognitive", "social", "other-psych", "non-psych"), 
  ...
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14), axis.title.x=element_blank())+
  labs(y="Count", fill="Subjective \n replication \n score")
...
open <- dat |> mutate(openness=case_when(
  ...
  geom_bar(stat="count", position="dodge")+
  scale_fill_viridis(discrete=T)+
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14), axis.title.x=element_blank(), legend.position="none")+
  labs(y="Count")

modal <- ggplot(dat, aes(x=ifelse(change_platform,"Switch to online","Same modality"), fill=sub_rep))+
  geom_bar(stat="count", position="dodge")+
  scale_fill_viridis(discrete=T)+
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14), axis.title.x=element_blank(), legend.position="none")+
  labs(y="Count")
...
within <- ggplot(dat, aes(x=ifelse(is_within, "Within", "Between"), fill=sub_rep))+
  geom_bar(stat="count", position="dodge")+
  scale_fill_viridis(discrete=T)+
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14), axis.title.x=element_blank(), legend.position="none")+
  labs(y="Count")
...
vignette <- ggplot(dat, aes(x=ifelse(single_vignette, "Single vignette", "Multiple vignettes"), fill=sub_rep))+
  geom_bar(stat="count", position="dodge")+
  scale_fill_viridis(discrete=T)+
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14), axis.title.x=element_blank(), legend.position="none")+
  labs(y="Count")

top_row <- plot_grid(subfield, modal, leg, rel_widths=c(1,.5,.5), nrow=1)
mid_row <- plot_grid(open, within, vignette, rel_widths=c(1,.5,.5), nrow=1)
