...
The identification of features that predict replication success is limited by data, however: most analyses draw on the same set of ~170 replications from a few replication projects that have been extensively re-analysed. 

We introduce a new dataset of 176 replications where we examine how often students in a graduate-level methods course were able replicate their chosen studies' results closely enough to be ready to build on it in future work. The replication rate under this metric was 49%, and 45% of replications with suitable numerical outcomes (N=136) were within the prediction interval of the original outcome. Larger original effect sizes and within-participants designs were predictive of replication. 
...
[Still needs work -- also needs shortening -- this is 180, NHB says max 150]
:::
...
<!--, both measures of whether the two effects are likely to have come from the same distribution with all the necessary compromises to the methods and sample of the original that may be required by the constraints of the situation.

[maybe drop last sentence and work into a longer paragraph?]

[TODO Maya says firm this up and introduce quant outcomes as well] 

As a complement, we also measure how similar/consistent the original and replication effects were for the key effect of each study. TODO blah blah
...
While a number of predictors show individual correlations with the subjective replication score, many of the predictors intercorrelate with one another. In order to determine which predictors were the strongest, we ran a series of pre-registered regression models regularized using a horseshoe prior (see Methods for details; see Supplement for all estimates from all models). We ran models both using all the original-replication pairs, but without the predictors original effect size and original p value (that were uncodable for some studies), and models including all predictors, but limiting to the subset of data where all predictors were available. The coefficient estimates from two models predicting the subjective replication scores are shown in Figure \ref{fig:mod-results}. Due to a large number of predictors coupled with a small and noisy dataset there is much uncertainty around the coefficients even with strong regularization. The general directions of coefficients are consistent with the effects of the predictors in isolation. 

Within-subjects designs stand out as the strongest indicator of replicability in the model with all the original-replication pairs (`r within_1`). In the model with all predictors, but less data, within-subjects designs remain predictive (`r within_3`). Standardized effect size is another strong predictor of subjective replication score (`r smd_3`). Both effects are robust to a sensitivity analysis including only studies with close replications and matching statistical tests (within-subjects `r within_sens`; effect size `r smd_sens`). 
...
The same issue that may cause a study not to replicate under constrained circumstances (despite hypothetically replicating under more favorable circumstances such as expert administration or larger budgets) will also plague attempts to build off those studies in constrained circumstances. Thus, we believe it is relevant to consider how well studies replicate under the limitations of real world resource and expertise constraints. 
