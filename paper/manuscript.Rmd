...
```{r, include=F}
matching <- d |>  mutate(match=ifelse(replicated_instructor_code==replicated_report_code,1,0)) |> group_by(match) |> tally()
...
```{r}
closeness <- d |> group_by(how_close) |> tally() |> mutate(pct=(n/sum(n)*100) |> round())

```

These replications were all conducted on short time scales, within a constrained class budget. In some cases the budget limited the number of participants who could be recruited, occasionally below what the original study included or what power analyses suggested. The replications had a median post-exclusion sample size that was `r round(ratio_dist[3]*100)`% of the original sample size.  In nearly all cases, replications were conducted online, with recruitment from Amazon Mechanical Turk (the default from 2011 to 2020) or Prolific (the default from 2021 -- 2022). The goal was for replications to be as close as possible to the original, but budgetary constraints, inability to option original materials, and primarily using online samples meant that replications varied in their degree of closeness to the original. According to the schema from @lebel2018, `r pluck(closeness,3,2)`% of studies were exact replications, `r pluck(closeness,3,4)`% were very close replications, `r pluck(closeness,3,1)`% were close replications, and `r pluck(closeness,3,3)` were far replications (See Methods for more details on common deviations).
...
for_cor <- d %>% filter(include %in% c("stats", "pred_int")) %>% 
  mutate(predInt=as.numeric(predInt))
sub_pred <- cor.test(for_cor$sub_rep, for_cor$predInt)$estimate
sub_p <- cor.test(for_cor$sub_rep, for_cor$p_orig)$estimate
pred_p <- cor.test(for_cor$predInt, for_cor$p_orig)$estimate



```

Across the 176 studies, the average subjective replication score was `r round(pluck(sub_rep_rate,1))`%, which we can interpret as an overall subjective replication rate. `r round(pluck(pred_int_rate,1))`% (`r predIntcount`/`r predInttotal`) of replications had outcomes with point-estimates within the prediction interval of the original outcome. The median *p*-original value on the original and replication point-estimates coming from the same distribution was `r round(pluck(porig_rate, 1),2)`, representing the median probability that a replication study's estimate would be at least as extreme as was actually observed, if in fact the replication and original were statistically consistent. The subjective replication scores were moderately correlated with both prediction interval (r=`r round(sub_pred,2)`) and *p*-original (r=`r round(sub_p, 2)`) values; prediction interval and *p*-original were highly correlated with each other (r=`r round(pred_p,2)`). As mentioned above, inconsistency between the subjective replication and numeric outcomes can occur in a few circumstances such as when there is a large precise original effect and a smaller, but clear replication effect; when there is a small, imprecise original effect and a smaller, null, or opposite replication effect; or when the key measure does not represent the overall pattern of the results.

```{r}
medians <- d %>% filter(include =="stats") %>%
  mutate(difference=target_d_calc-rep_d_calc) %>% summarize(orig=quantile(target_d_calc), rep=quantile(rep_d_calc), diff=quantile(difference))

original_50  <- pluck(medians, 1,3) %>% round(2)
original_25 <- str_c(pluck(medians, 1,2) %>% round(2)," - ",pluck(medians, 1,4) %>% round(2))

rep_50  <- pluck(medians, 2,3) %>% round(2)
rep_25 <- str_c(pluck(medians, 2,2) %>% round(2)," - ",pluck(medians, 2,4) %>% round(2))

diff_50 <- pluck(medians, 3,3) %>% round(2)
diff_25 <- str_c(pluck(medians, 3,2) %>% round(2)," - ",pluck(medians, 3,4) %>% round(2))

medians_successful <- d %>% filter(include =="stats") %>%
  filter(sub_rep %in% c(1)) %>% 
    mutate(diff=target_d_calc-rep_d_calc) %>%
  summarize(orig=quantile(target_d_calc), rep=quantile(rep_d_calc),
            diff=quantile(diff))

original_50_succ  <- pluck(medians_successful, 1,3) %>% round(2)
original_25_succ <- str_c(pluck(medians_successful, 1,2) %>% round(2)," - ",pluck(medians_successful, 1,4) %>% round(2))

rep_50_succ <- pluck(medians_successful, 2,3) %>% round(2)
rep_25_succ <- str_c(pluck(medians_successful, 2,2) %>% round(2)," - ", pluck(medians_successful, 2,4) %>% round(2))

diff_50_succ <- pluck(medians_successful, 3,3) %>% round(2)
diff_25_succ <- str_c(pluck(medians_successful, 3,2) %>% round(2)," - ",pluck(medians_successful, 3,4) %>% round(2))

n_successful <- d %>% filter(include =="stats") %>%
  filter(sub_rep %in% c(1)) %>% nrow()
```
...
On average, there was a diminution of effect sizes from original to replication. This pattern of results is consistent with the results of RP:P [@openscienceconsortium2015]. Specifically, the median original effect size (in SMD units) was `r original_50` (interquartile range: `r original_25`), and the median replication effect size was `r rep_50` (`r rep_25`), for a median difference of `r diff_50` (`r diff_25`). Among the pairs with a subjective replication score of 1 (N=`r n_successful` with standardized effect sizes), the median original effect size was `r original_50_succ` (interquartile range: `r original_25_succ`), and the median replication effect size was `r rep_50_succ` (`r rep_25_succ`), for a median difference of `r diff_50_succ` (`r diff_25_succ`). Thus, for the highest subjectively-rated replications, effect sizes for original and replication were similar in magnitude on average, with some replications having larger effects than their originals. 
...
## Bivariate correlates of replication success
...
desc <- d %>% mutate(target_d_calc=ifelse(include=="stats", target_d_calc, NA),
             target_p_calc=ifelse(include=="stats",target_p_calc, NA),
      switch_modality=ifelse(target_on_turk!=replication_on_turk,1,0),
         ss_ratio=replication_N/target_N)%>% 
  select("subfield","open_data", "open_materials","switch_modality",  "stanford_internal","within_between","single_vignette","repeated_measure","target_year","target_N","replication_N", "ss_ratio", "target_d_calc", "target_p_calc") %>% 
  mutate(subfield=factor(subfield, levels=c("cognitive", "social", "other-psych", "non-psych"), labels=c("Cognitive", "Social", "Other psych", "Non-psych")),
         within_between=ifelse(within_between=="between", 0, 1)) %>% 
             tbl_summary(
               label=list(
                 subfield ~ "Subfield",
                 stanford_internal ~ "Original authors at Stanford",
                 target_year~ "Publication Year",
                 single_vignette ~ "Single vignette (1 item/condition)",
                 open_data ~ "Open data",
                 open_materials ~ "Open materials",
                 within_between ~ "Within participants design",
                 repeated_measure ~ "Number of trials",
                 target_N ~ "Original sample size",
                 replication_N ~ "Replication sample size",
                 ss_ratio ~ "Ratio of replication/original sample sizes",
                 target_d_calc ~"Original effect size (SMD)",
                 target_p_calc ~ "Original p-value",
                 switch_modality ~ "Switch from in-person to online"
               ),
               statistic = list(all_continuous() ~ "{mean} ({sd})", all_continuous() ~ "{median} ({p25}, {p75})"),
               digits=list(target_year ~ 0, target_p_calc ~4),
    missing="no",
             ) %>% modify_header(label="Feature", stat_0="Summary") %>% 
    modify_footnote(update = everything() ~ NA)

as_kable_extra(desc,format="latex", position="!h", caption="Descriptive properties of the replication dataset. For categorical features, counts and percentages are given. For continuous features, the median value and interquartile range are given. N=176 except for original effect size and p-value where N=112. See Methods for full description of how these values were coded. ", booktabs=TRUE, linesep="") |> kable_styling(full_width=F, font_size=10)
...
within <- for_cor |> group_by(is_within) |> summarize(avg=mean(sub_rep))

cog <- for_cor |> group_by(subfield) |> summarize(avg=mean(sub_rep))
vignette <- for_cor |> group_by(single_vignette) |> summarize(avg=mean(sub_rep))

```
Distributions of study outcomes across some of these properties are shown in Figure \ref{fig:predictors-graph}. Both social and cognitive psychology studies were well represented in the replication sample, and the cognitive psychology studies replicated more often than social psychology studies (mean subjective replication scores: `r pluck(cog,2,1) |> round(2)` versus `r pluck(cog,2,4) |> round(2)`). Within and between participants designs were both common, and within-participants designs replicated more often than between-participants designs (mean scores: `r pluck(within,2,2) |> round(2)` versus `r pluck(within,2,1) |> round(2)`). Studies with multiple vignettes replicated more often than single vignetted studies (mean scores: `r pluck(vignette,2,1) |> round(2)` versus `r pluck(vignette,2,2) |> round(2)`). However, there were strong correlations among these experimental features as well as between these experimental features and specific subfields (Figure \ref{fig:corr-plot}). 
...
```{r corr-plot, out.height="40%", fig.width=8, fig.height=4.5, fig.pos="ht", fig.cap="Correlations between predictor variables. See Methods for descriptions of how each variable was coded. "}
dat <-  for_cor %>% mutate(cognitive=ifelse(subfield=="cognitive",1,0)) %>% select(-sub_rep, -subfield) %>% select( "social", "cognitive", "other_psych", "non_psych","is_within", "single_vignette", "stanford", "change_platform", "open_data", "open_mat", "pub_year", "log_trials", "log_sample", "log_ratio_ss", )
...
melted_corr_mat <-melt(corr_mat) %>% 
  mutate(value=ifelse(Var1==Var2, NA, value),
         value=ifelse(Var1 %in% c("cognitive", "non_psych", "other_psych", "social") &
                        Var2 %in% c("cognitive", "non_psych", "other_psych", "social"),
                      NA, value)) %>% 
    filter(Var1!="log_ratio_ss") |> 
  mutate(Var1=factor(Var1, levels=c("social", "cognitive", "other_psych", "non_psych", "is_within", "single_vignette", "stanford", "change_platform", "open_data", "open_mat", "pub_year", "log_trials", "log_sample", "log_ratio_ss"), labels=c("Social psych", "Cognitive psych", "Other psych", "Non-psych", "Within participants design", "Single vignette ","Stanford affiliated", "Switch to online", "Open data", "Open materials", "Publication year", "Log # of trials", "Log original sample size", "Log ratio rep/original sample sizes"))) |> 
           filter(!Var2 %in% c("social", "cognitive","other_psych", "non_psych")) |> 
         mutate(Var2=factor(Var2, levels=c("social", "cognitive", "other_psych", "non_psych", "is_within", "single_vignette", "stanford", "change_platform", "open_data", "open_mat", "pub_year", "log_trials", "log_sample", "log_ratio_ss"), labels=c("Social psych", "Cognitive psych", "Other psych", "Non-psych", "Within participants design", "Single vignette ","Stanford affiliated", "Switch to online", "Open data", "Open materials", "Publication year", "Log # of trials", "Log original sample size", "Log ratio rep/original sample sizes"))) 
...
ggplot(data = melted_corr_mat, aes(x=Var1, y=Var2,
  ...
  scale_fill_gradient2(low="#763386",high="#FDE725FF", na.value="grey")+
  labs(x="", y="")+
  geom_tile(alpha=.7)+
...
do_estimates <- function(model, variable){
  summary(model)$fixed |> as_tibble(rownames="var") |> filter(var==variable)|> 
    mutate(OR=exp(Estimate), lower=exp(`l-95% CI`), upper=exp(`u-95% CI`)) |> 
    mutate(text=str_c("OR = ", round(OR,2), ",  95\\% CrI = [", round(lower,2),", ", round(upper,2),"]")) |> pull(text)
}
...
```{r mod-results, out.width="90%", fig.width=8, fig.height=5, fig.pos="ht", fig.cap="Odds ratios and corresponding 95\\% CrI on the likelihood of having a higher subjective replication score as a function of the independent variable. Estimates from a model of all original-replication pairs (N=176) are shown in blue, and from a model of all pairs with full statistical information (N=112) are shown in red. A value of 1 indicates no association, greater than one indicates an association with higher replication scores and less than 1 an association with lower replication scores."}
...
tier1_subjective <- read_rds(here(model_location, "tier1_subjective.rds")) |> do_draws() |> mutate(OR=exp(Estimate))|> mutate(type="All pairs") |> mutate(sens="All data")
tier3_subjective <- read_rds(here(model_location, "tier3_subjective.rds"))|> do_draws() |>  mutate(OR=exp(Estimate)) |> mutate(type="Pairs with full statistical information") |> mutate(sens="All data")

tier1_subjective |> union(tier3_subjective) |> ggplot(aes(y = reorder(Term, OR, mean), x = OR, color=type)) + geom_vline(xintercept=1)+
  scale_color_solarized()+
  stat_pointinterval(position=position_dodge(width=.7))+labs(y="", x="Odds Ratios for subjective replication score (point estimate, 66% CrI, 95% CrI)", color="", linetype="", shape="")+theme(axis.text=element_text(size=11),legend.position="none")+coord_trans(x="log10")+scale_x_continuous(breaks=c(.125, .25, .5, 1, 2, 4, 8, 16), minor_breaks = NULL)
